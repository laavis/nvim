...
require('lazy').setup({
      ...
      require('mason-lspconfig').setup {
        ...
        handlers = {
          function(server_name)
            ...
            -- Check if it's tsserver, then modify its settings
            if server_name == 'tsserver' then
              server.settings = {
                completions = {
                  completeFunctionCalls = false, -- Prevents overloading completion with function calls
                },
              }
            end

            -- This ensures capabilities are merged properly
    ...
    config = function()
      ...
      cmp.setup {
        ...
        sources = cmp.config.sources({
          ...
          { name = 'buffer', keyword_length = 4 }, -- Require 4+ chars before suggesting from buffer
        }),
      }
    end,
  },
  ...
  { -- Highlight, edit, and navigate code
    ...
    opts = {
      ...
      auto_install = true,
      injections = {
        enable = true, -- Ensure injections are enabled
      },
      highlight = {
        enable = true,
